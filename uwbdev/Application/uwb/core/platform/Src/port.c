...
bool get_button_1_state(){
	return button1;
}

void reset_button_1_state(){
	button1 = false;
}

bool get_button_2_state(){
	return button2;
}

void reset_button_2_state(){
	button2 = false;
}
...
{
    return EXTI_GetITEnStatus(EXTI9_5_IRQn);
}
...
void setup_DWICRSTnIRQ(int enable)
{
    ...
    if(enable)
    {
        ...
        HAL_NVIC_EnableIRQ(EXTI9_5_IRQn);     //pin #0 -> EXTI #0
        HAL_NVIC_SetPriority(EXTI9_5_IRQn, 5, 0);
    }
    else
    {
        HAL_NVIC_DisableIRQ(EXTI9_5_IRQn);    //pin #0 -> EXTI #0
